...
class blogCore{
	...
	</head><body class="markdown-body editormd-preview-container">';
public $htmlBody = '</body></html>';
    ...
    function updateFile($request,$format){
        Storage::put($request->input('path'), $request->input('doc'));
	Storage::put($request->input('path').'.html', $this->htmlHead.$request->input('test-editor-html-code').$this->htmlBody);
	$format->code = 1;
